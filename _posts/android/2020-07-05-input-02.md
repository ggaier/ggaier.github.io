...
其中第二个问题不想说了，暂不作为本文的重点，在文章中有提及，但会暂时略过。
...
4. 最后InputPublisher，把数据通过序列化的方法，写入到InputDispatcher监听的InputChannel中，以此来唤醒InputDisptcher接受输入事件的回调，开始分发事件。

那么第一个问题就差不多回答完了，只有一部分-也就是`InputReader`是如何从各种输入设备取出输入数据的-暂不清楚，也暂时不需要了解那么详细了，就略过吧。